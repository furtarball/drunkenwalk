...
Wrapped<SDL_Texture> Renderer::textTexture(const string& t, Font f,
										   SDL_Rect& dim) {
...
void Renderer::print(const string& t, Font f, SDL_Rect& dst, char vAlign,
					 char hAlign) {
...
void Renderer::drawOSD(shared_ptr<Player> player, array<unsigned, 2>& seed) {
	...
	text << "Map seed: {" << hex << seed[0] << dec << "; " << seed[1] << "}\n\n";
